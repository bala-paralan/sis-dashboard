    ...
    >
      ...
      <div className="flex items-center gap-1.5 py-1 px-[10px] rounded-[6px] bg-bg-tertiary border border-border-color shrink-0">
        ...
        <span className="topbar-user-label text-[11px] text-text-primary font-semibold">
          {authUser?.displayName ?? authUser?.email ?? 'Operator'}
        </span>
      </div>
      {authUser && (
        <button
          ...
          title="Sign out"
          aria-label="Sign out"
          className="w-[30px] h-[30px] flex items-center justify-center rounded-[6px] border border-border-color bg-bg-tertiary text-text-secondary text-[13px] cursor-pointer shrink-0 hover:text-alert-critical hover:border-alert-critical transition-colors"
        >
          ⏻
        </button>
      )}
