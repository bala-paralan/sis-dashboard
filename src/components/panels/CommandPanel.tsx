...
export function CommandPanel() {
  ...
  function handleExportIncident() {
    const onlineNodes = nodes.filter((n) => n.status === 'ONLINE')
    const threatenedNodes = nodes.filter((n) => n.threatLevel !== 'CLEAR')
    printReport({
      title: 'Incident Report',
      subtitle: 'BHQN Submission — Unclassified',
      site: 'BOP-ALPHA-01',
      timestamp: new Date().toLocaleString(),
      sections: [
        { label: 'Date / Time', value: new Date().toLocaleString() },
        { label: 'Reporting Node', value: 'BOP-ALPHA-01' },
        { label: 'Nodes Online', value: `${onlineNodes.length} / ${nodes.length}` },
        { label: 'Active Alerts', value: String(totalAlerts) },
        { label: 'Threat Level', value: threatenedNodes.length > 0 ? threatenedNodes[0].threatLevel : 'CLEAR' },
        { label: 'Sensor Faults', value: nodes.filter((n) => n.status === 'DEGRADED').map((n) => n.id).join(', ') || 'None' },
      ],
      narrative: incidentText || '(No narrative provided)',
      narrativeLabel: 'Incident Narrative',
      footer: 'Generated by IINVSYS SIS Dashboard — STANAG 4607/4609 compliant',
    })
  }
  ...
  function handleExportHandover() {
    printReport({
      ...
      timestamp: new Date().toLocaleString(),
      sections: [
        { label: 'Period', value: 'Last 12 hours' },
        { label: 'Total Alerts', value: String(totalAlerts + 12) },
        { label: 'Acknowledged', value: '8' },
        { label: 'Pending Alerts', value: String(totalAlerts + 4) },
        { label: 'Sensor Faults', value: 'BOP-BETA-01: ACOUSTIC S04, BOP-DELTA-01: RADAR S12' },
        { label: 'Tracks Detected', value: '7 (5 ANIMAL, 1 HUMAN, 1 UNKNOWN)' },
        { label: 'UAS Contacts', value: '1 (commercial, 450 m range, logged)' },
        { label: 'GPR Anomalies', value: '3 flagged for field verification' },
        { label: 'Nodes Online', value: `${nodes.filter((n) => n.status === 'ONLINE').length} / ${nodes.length}` },
      ],
      narrative: handoverNotes || '(No handover notes provided)',
      narrativeLabel: 'Handover Notes',
      footer: 'Operator signature required before filing — IINVSYS SIS Dashboard',
    })
  }
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {tab === 'incident' && showIncident && (
          <div className="p-[10px]">
            ...
            <div className="flex gap-1.5 mt-2">
              <button onClick={handleExportIncident} className="flex-1 py-1.5 bg-accent-blue border-none rounded text-white cursor-pointer text-[10px] font-bold">
                ⬇ Export PDF → BHQN
              </button>
        ...
        {tab === 'handover' && showHandover && (
          <div className="p-[10px]">
            ...
            />
            <div className="flex gap-1.5 mt-2">
              <button onClick={handleExportHandover} className="flex-1 py-1.5 bg-accent-blue border-none rounded text-white cursor-pointer text-[10px] font-bold">
                ✍ Sign &amp; Export PDF
              </button>
            </div>
          </div>
