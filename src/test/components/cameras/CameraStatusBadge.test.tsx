...
describe('CameraStatusBadge', () => {
  it('renders without crashing', () => {
    expect(() => render(<CameraStatusBadge status="ONLINE" />)).not.toThrow()
  })

  it.each(STATUSES)('renders the status text for %s', (status) => {
  ...
  it('renders a colour dot indicator inside the badge', () => {
    const { container } = render(<CameraStatusBadge status="ONLINE" />)
